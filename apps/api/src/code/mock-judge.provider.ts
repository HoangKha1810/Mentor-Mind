...
export class MockJudgeProvider implements CodeJudgeProvider {
  ...
  async run(input: JudgeRequest): Promise<JudgeResult> {
    ...
    const syntacticallySuspicious =
      input.code.trim().length < 12 || /\bthrow\s+new\b|\bTODO\b/i.test(input.code);
    ...
    return {
      ...
      publicResults: input.testCases
        ...
        .map((test, index) => ({
          ...
          verdict: verdict === CodeVerdict.ACCEPTED || index > 0 ? CodeVerdict.ACCEPTED : verdict,
          statusDescription:
            verdict === CodeVerdict.ACCEPTED || index > 0 ? 'Accepted' : 'Mock failure',
          runtimeMs: 24 + Math.min(input.code.length, 200),
          memoryKb: 12_288,
          errorMessage:
            verdict === CodeVerdict.ACCEPTED || index > 0
              ? undefined
              : verdict === CodeVerdict.RUNTIME_ERROR
                ? 'Mock judge detected incomplete or intentionally failing code.'
                : 'Mock judge simulated a wrong answer on at least one test.',
