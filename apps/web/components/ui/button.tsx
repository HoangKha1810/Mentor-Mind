...
export const Button = forwardRef<HTMLButtonElement, ButtonProps>(
  (
    ...
    const inactive = disabled || ariaDisabled === true || ariaDisabled === 'true';
    ...
    return (
      <motion.button
        ...
        disabled={disabled}
        aria-disabled={ariaDisabled}
        whileHover={inactive ? undefined : { y: -2, scale: 1.012 }}
        whileTap={inactive ? undefined : { scale: 0.98, y: 0 }}
        transition={motionSpring.interaction}
        data-motion-button="true"
        className={cn(
          'motion-button relative inline-flex shrink-0 items-center justify-center gap-2 overflow-hidden whitespace-nowrap rounded-full border font-semibold tracking-normal outline-none transition-[background-color,border-color,color,box-shadow] duration-200 focus-visible:ring-2 focus-visible:ring-success/60 focus-visible:ring-offset-2 focus-visible:ring-offset-background aria-disabled:pointer-events-none aria-disabled:cursor-not-allowed aria-disabled:opacity-55 disabled:pointer-events-none disabled:cursor-not-allowed disabled:opacity-55',
          variant === 'primary' &&
