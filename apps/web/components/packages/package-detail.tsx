...
export function PackageDetail({ slug }: { slug: string }) {
  ...
  if (!query.data) {
    return <EmptyState title="Không tìm thấy gói học" description="Gói học này chưa được publish hoặc đã bị lưu trữ." />;
  }
    ...
    <div className="grid gap-6 lg:grid-cols-[1.1fr_0.9fr]">
      <Card>
        ...
        <CardHeader>
          ...
          <CardDescription>{query.data.longDescription ?? query.data.shortDescription}</CardDescription>
        </CardHeader>
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>Gửi yêu cầu tư vấn để đội ngũ xác nhận mục tiêu và lịch học.</CardDescription>
        </CardHeader>
        <form onSubmit={requestConsultation} className="space-y-4">
          <Textarea name="message" placeholder="Mục tiêu, deadline, nền tảng hiện tại và khung giờ mong muốn" required />
