...
const nodes = ['Code', 'Interview', 'Roadmap', 'Mentor', 'Resources', 'CV'];
  ...
  const nodePositions = useMemo(
    () =>
      nodes.map((_, index) => {
        ...
        return new THREE.Vector3(Math.cos(angle) * 2.5, Math.sin(angle * 1.4) * 0.28, Math.sin(angle) * 1.55);
      }),
    [],
  );
    ...
    <group ref={group}>
      ...
      {nodePositions.map((position, index) => (
        <group key={nodes[index]} position={position}>
          <Line points={[new THREE.Vector3(0, 0, 0), position.clone().multiplyScalar(-1)]} color="#00D4FF" opacity={0.18} transparent />
          <Sphere args={[0.22, 32, 32]}>
            <meshStandardMaterial color={index % 2 ? '#00D4FF' : '#22C55E'} emissive={index % 2 ? '#00D4FF' : '#22C55E'} emissiveIntensity={0.35} />
          </Sphere>
          <Text
            position={[0, -0.44, 0]}
            fontSize={0.16}
      ...
      {Array.from({ length: 34 }).map((_, index) => {
        ...
        return (
          <Sphere key={index} args={[0.015, 12, 12]} position={[Math.cos(angle) * radius, Math.sin(index) * 1.1, Math.sin(angle) * radius]}>
...
export function LearningOrbit() {
  return (
    <div className="relative min-h-[360px] overflow-hidden rounded-lg border border-white/10 bg-surface shadow-glow md:min-h-[520px]">
      ...
      <Canvas camera={{ position: [0, 1.2, 5.4], fov: 45 }} dpr={[1, 1.5]} gl={{ antialias: true, alpha: true }}>
