          ...
          <ScrollReveal delay={0.2} direction="up">
            <StaggerContainer className="grid gap-4 md:grid-cols-2 lg:grid-cols-4">
              {publicFeatures.map((feature, index) => {
                ...
                return (
                  <StaggerItem key={feature.title} className={index === 0 ? 'lg:col-span-2' : ''}>
                    <Link href={feature.href} className="group block h-full">
                      <Card interactive reveal={false} className="h-full min-h-[14rem]">
                        <div
                          className={`pointer-events-none absolute inset-0 bg-gradient-to-br ${tone.panel}`}
                        />
        ...
        <section className="process-band relative border-y border-white/8">
          <div className="mx-auto max-w-7xl px-4 py-14 sm:px-6 lg:px-8">
            <ScrollReveal direction="up">
              ...
              <h2 className="mt-2 text-3xl font-semibold text-white">
                Từ mục tiêu đến tiến độ đo được
              </h2>
            </ScrollReveal>
          ...
          <div className="mx-auto max-w-7xl px-4 py-16 text-center sm:px-6 lg:px-8 lg:py-20">
            <ScrollReveal direction="up">
              ...
              <p className="mx-auto mt-5 max-w-3xl text-base leading-8 text-mutedText">
                MentorMind giúp bạn biến mục tiêu nghề nghiệp thành lộ trình học rõ ràng, luyện
                code, hoàn thiện CV và chuẩn bị phỏng vấn trong một hệ thống có AI ghi nhớ ngữ cảnh
                học tập của từng tài khoản.
              </p>
            </ScrollReveal>
            ...
            <ScrollReveal delay={0.2} direction="up">
              <div className="mx-auto mt-14 max-w-3xl">
                ...
                <p className="mt-6 text-base leading-8 text-mutedText">
                  ...
                  thống dùng làm chuẩn tham chiếu để gợi ý bài luyện, CV keyword và câu hỏi phỏng
                  vấn.
                </p>
                <Link
...
const showcaseTiles = [
  {
    ...
    href: '/code-practice',
    description:
      'Danh sách 100 bài, bài đặc biệt, trạng thái accepted và editor gửi ngữ cảnh sang trợ lý AI.',
    icon: Code2,
    ...
    preview: (
      <div className="space-y-2 text-xs text-white">
        {['Câu hỏi 1', 'Voice input', 'Điểm 8/10'].map((item) => (
          <div key={item} className="rounded-lg bg-white/14 px-3 py-2">
            {item}
          </div>
        ))}
      </div>
    ...
    preview: (
      <div className="flex flex-wrap gap-2 text-xs font-semibold text-white">
        {['React', 'AI Agent', 'CV', 'Interview'].map((item) => (
          <span key={item} className="rounded-full bg-white/16 px-3 py-1">
            {item}
          </span>
        ))}
      </div>
